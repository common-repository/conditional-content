...
class Logger {
	...
	private static function getConfigurator($configurator = null) {
		...
		if (is_object($configurator)) {
			if ($configurator instanceof \com\extremeidea\php\tools\log4php\LoggerConfigurator) {
				return $configurator;
			} else {
		...
		if (is_string($configurator)) {
			...
			if (!($instance instanceof \com\extremeidea\php\tools\log4php\LoggerConfigurator)) {
