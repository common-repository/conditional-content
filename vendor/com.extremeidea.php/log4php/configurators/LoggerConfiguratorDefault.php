...
class LoggerConfiguratorDefault implements LoggerConfigurator
{
	...
	private $adapters = array(
		self::FORMAT_XML => '\com\extremeidea\php\tools\log4php\LoggerConfigurationAdapterXML',
		self::FORMAT_INI => '\com\extremeidea\php\tools\log4php\LoggerConfigurationAdapterINI',
		self::FORMAT_PHP => '\com\extremeidea\php\tools\log4php\LoggerConfigurationAdapterPHP',
	);
	...
	private static $defaultConfiguration = array(
        ...
        'appenders' => array(
            'default' => array(
                'class' => '\com\extremeidea\php\tools\log4php\LoggerAppenderEcho'
            ),
        ),
	);
	...
	private function configureAppender($name, $config) {
		...
		if (!($appender instanceof \com\extremeidea\php\tools\log4php\LoggerAppender)) {
		...
		if (isset($config['threshold'])) {
			$threshold = LoggerLevel::toLevel($config['threshold']);
			if ($threshold instanceof \com\extremeidea\php\tools\log4php\LoggerLevel) {
	...
	private function createAppenderLayout(LoggerAppender $appender, $config) {
		...
		$layout = new $class();
		if (!($layout instanceof \com\extremeidea\php\tools\log4php\LoggerLayout)) {
	...
	private function createAppenderFilter(LoggerAppender $appender, $config) {
		...
		$filter = new $class();
		if (!($filter instanceof \com\extremeidea\php\tools\log4php\LoggerFilter)) {
