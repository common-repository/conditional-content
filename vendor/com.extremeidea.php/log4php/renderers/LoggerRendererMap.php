...
class LoggerRendererMap {
	...
	public function addRenderer($renderedClass, $renderingClass) {
		...
		if (!class_exists('\\com\\extremeidea\\php\\tools\\log4php\\' . $renderingClass)) {
			trigger_error("log4php: Failed adding renderer. Rendering class [$renderingClass] not found.");
			return;
		}

        //Fix for namespaced version
		$renderingClass = '\\com\\extremeidea\\php\\tools\\log4php\\' . $renderingClass;

		// Create the instance
		$renderer = new $renderingClass();

		// Check the class implements the right interface
		if (!($renderer instanceof \com\extremeidea\php\tools\log4php\LoggerRenderer)) {
